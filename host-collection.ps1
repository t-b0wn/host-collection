...
foreach ($ip in $targ_ips){
    ...
    if($ipsession){
        ...
        $host_hashes | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_hashes.txt") -Encoding ascii
        ...
        $host_netstat | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_netstat.txt") -Encoding ascii
        ...
        $host_process | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_process.txt") -Encoding ascii
        ...
        $host_registry | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_registry.txt") -Encoding ascii
        ...
        $host_firewall | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_firewall.txt") -Encoding ascii
        ...
        $host_smbshare | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_smbshare.txt") -Encoding ascii
        ...
        $host_sysinfo | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_sysinfo.txt") -Encoding ascii
        ...
        foreach ($service in $host_services) {
            $allservices += (@{
                servicename=$service.Name
                displayname=$service.DisplayName
                imagepath=get-itempropertyvalue -path "HKLM:\SYSTEM\CurrentControlSet\Services\$($service.Name)\" -name ImagePath
                servicedll=get-itempropertyvalue -path "HKLM:\SYSTEM\CurrentControlSet\Services\$($service.Name)\Parameters" -name ServiceDLL -ErrorAction Ignore
                status=$service.Status
                starttype=$service.StartType
            })
        }

        $allservices | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_services.txt") -Encoding ascii
        #$host_services | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_services.txt") -Encoding ascii
        ...
        $host_netconfig | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_netconfig.txt") -Encoding ascii
        ...
        $host_schedtasks | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_schedtasks.txt") -Encoding ascii
        ...
        $host_users | ConvertTo-Json | out-file ($ip.tostring() + "_" + $timestamp + "_users.txt") -Encoding ascii
